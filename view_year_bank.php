...
if(!empty($year) && !empty($type))//если переменная существует
{
	$query_cat = "year='$year' AND type='$type'";//И страна = той стране, которую мы выбрали и тому типу 
	$cat_link = "year=$year&";
}
else
{
	...
	if(!empty($year))//если существует только тип(в ссылке), то выводит все этого типа
	{
		$cat_link = "year='.$year.'&";
	}
	else
...
$result = mysql_query("SELECT * FROM products INNER JOIN banknotes ON products.id_products = banknotes.id_products WHERE $query_cat  AND products.count > 0 AND products.type='bancnots' ORDER BY $sorting $query_start_num",$link) or die(mysql_error());
	...
	<div id="block-sorting">
		...
		<ul id="option-list">
			...
			<li><a id="select-sort">'.$sort_name.'</a>
					<ul id="sorting-list">
						<li><a href="view_year_bank.php?'.$cat_link.'type='.$type.'&sort=price-asc">От дешевых к дорогим</a></li><!--sort-переменная, в которой будет хранится название сортировки-->
						<li><a href="view_year_bank.php?'.$cat_link.'type='.$type.' & sort=price-desc">От дорогих к дешевым</a></li>
					</ul>
...
if ($page != 1){ $pstr_prev = '<li><a class="pstr-prev" href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page - 1).'">&lt;</a></li>';}//для ссылки "назад", если ссылка не 1, то кнопка есть и работает
if ($page != $total) $pstr_next = '<li><a class="pstr-next" href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page + 1).'">&gt;</a></li>';//для ссылки "вперед", если не есть общее кол-во страница, то выводим 
 
 
// Формируем ссылки со страницами
if($page - 5 > 0) $page5left = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page - 5).'">'.($page - 5).'</a></li>';//если результат больше нуля, то формируются ссылки
if($page - 4 > 0) $page4left = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page - 4).'">'.($page - 4).'</a></li>';
if($page - 3 > 0) $page3left = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page - 3).'">'.($page - 3).'</a></li>';
if($page - 2 > 0) $page2left = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page - 2).'">'.($page - 2).'</a></li>';
if($page - 1 > 0) $page1left = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page - 1).'">'.($page - 1).'</a></li>';
 
if($page + 5 <= $total) $page5right = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page + 5).'">'.($page + 5).'</a></li>';//если результат менье общего кол-ва страниц, то формируются ссылки
if($page + 4 <= $total) $page4right = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page + 4).'">'.($page + 4).'</a></li>';
if($page + 3 <= $total) $page3right = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page + 3).'">'.($page + 3).'</a></li>';
if($page + 2 <= $total) $page2right = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page + 2).'">'.($page + 2).'</a></li>';
if($page + 1 <= $total) $page1right = '<li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.($page + 1).'">'.($page + 1).'</a></li>';

//Проверка на вывод многоточиё общего вывода страниц
if ($page+5 < $total)
{
    $strtotal = '<li><p class="nav-point">...</p></li><li><a href="view_year_bank.php?cat='.$year.'&type='.$type.'&page='.$total.'">'.$total.'</a></li>';
}else
{
    $strtotal = ""; 
}
...
if ($total > 1)
{
    ...
    <ul>
    ';
    echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class='pstr-active' href='view_year_bank.php?cat='.$year.'&type='.$type.'&page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
    echo '
    </ul>
    </div>
    ';
