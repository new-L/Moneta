...
<a href="spr_system/spr_system.pdf" id="spr-system"><img src="images/spr-system.png" alt="Справочная система"></a><!--download-->
...
<?php 
 $num = 3;//указываем, сколько хотим выводить товаров на сайт
 $page = (int)$_GET['page'];

 $count = mysql_query("SELECT COUNT(*) FROM `products`",$link);//подсчет всех товаров
...
if($temp[0] > 0)//если в переменной есть товары то.....
{
	...
	$total = (($tempcount - 1)/($num * 3))+1;//кол-во товаров - 1, чтобы если число нецелое, чтобы выводился остаток на следующей странице
...
//ВЫВОД ТОВАРОВ
//1)SQL-запрос
$result = mysql_query("SELECT * FROM products INNER JOIN coins  ON products.id_products = coins.id_products  WHERE products.count > 0 AND products.type='monets' ORDER BY $sorting $query_start_num ",$link) or die(mysql_error());
$result_bank = mysql_query("SELECT * FROM products INNER JOIN banknotes ON products.id_products = banknotes.id_products WHERE products.count > 0 AND products.type='bancnots' ORDER BY $sorting $query_start_num",$link)or die(mysql_error());
$result_rew = mysql_query("SELECT * FROM products INNER JOIN rewards ON products.id_products = rewards.id_products  WHERE products.count > 0 AND products.type='rewards' ORDER BY $sorting $query_start_num",$link)or die(mysql_error());
...
if ($page != 1){ $pstr_prev = '<li><a class="pstr-prev" href="index.php?page='.($page - 1).'">&lt;</a></li>';}//для ссылки "назад", если ссылка не 1, то кнопка есть и работает
if ($page != $total) $pstr_next = '<li><a class="pstr-next" href="index.php?page='.($page + 1).'">&gt;</a></li>';//для ссылки "вперед", если не есть общее кол-во страница, то выводим 
 
 
// Формируем ссылки со страницами
if($page - 5 > 0) $page5left = '<li><a href="index.php?page='.($page - 5).'">'.($page - 5).'</a></li>';//если результат больше нуля, то формируются ссылки
...
if($page + 5 <= $total) $page5right = '<li><a href="index.php?page='.($page + 5).'">'.($page + 5).'</a></li>';//если результат менье общего кол-ва страниц, то формируются ссылки
